...
public final class LoginController {
    ...
    public func landing(request: Request) -> Response {
        do {
            ...
            return Response(redirect: "/dashboard").flash(.success, "Logged in as \(user.email)")
        } catch {
    ...
    public func resetPasswordSubmit(request: Request) -> Response {
        do {
            guard let email = request.data["email"]?.string, let backendUser: BackendUser = try BackendUser.makeQuery().filter("email", email).first() else {
                return Response(redirect: "/login").flash(.success, "E-mail with instructions sent if user exists")
            }
            ...
            return Response(redirect: "/login/reset").flash(.error, "Error occurred")
        }
    }
    ...
    public func resetPasswordTokenSubmit(request: Request) throws -> Response {
        ...
        if(password != passwordRepeat) {
            return Response(redirect: "/login/reset/" + tokenStr).flash(.error, "Passwords did not match")
        }

        if !password.passes(Count.min(8)) {
            return Response(redirect: "/login/reset/" + tokenStr).flash(.error, "Passwords did not match requirement")
        }
        ...
        return Response(redirect: "/login").flash(.success, "Password is reset")
    }
    ...
    public func submit(request: Request) throws -> Response {
        ...
        do {
            ...
            return Response(redirect: "/login").flash(.error, "Failed to login")
        }
    }
